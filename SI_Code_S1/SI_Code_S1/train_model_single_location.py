"""
Single-Location Baseline CNN Training
Comparison baseline for Multi-branch approach
Uses only ONE auscultation location (AV - Aortic Valve)
"""
...
class Config:
    ...
    OUTPUT_DIR = r"D:\the-circor-digiscope-phonocardiogram-dataset-1.0.3\model_outputs_single"
...
# ============================================================================
# Single-Location Dataset
# ============================================================================
class SingleLocationDataset(Dataset):
    def __init__(self, data_df, data_dir, target_location, augment=False):
    ...
    def _prepare_samples(self):
        ...
        for idx, row in self.data_df.iterrows():
            ...
            if (pd.notna(row['Systolic murmur timing']) and 
                ...
                if self.target_location in locations:
    ...
    def __getitem__(self, idx):
        ...
        file_path = os.path.join(self.data_dir, f"{patient_id}_{self.target_location}.wav")
...
# ============================================================================
# Single-Location CNN Model
# ============================================================================
class SingleLocationCNN(nn.Module):
    def __init__(self, num_timing_classes, num_shape_classes, num_grading_classes):
        super(SingleLocationCNN, self).__init__()
    ...
    def forward(self, x):
        ...
        return self.timing_head(features), self.shape_head(features), self.grading_head(features)
...
def plot_confusion_matrices(results, encoders, save_path):
    ...
    sns.heatmap(cm_t, annot=True, fmt='d', cmap='Reds', ax=axes[0],
                xticklabels=encoders[0].classes_, yticklabels=encoders[0].classes_)
    axes[0].set_title('Timing (Single-Location)', fontsize=14, fontweight='bold')
    axes[0].set_xlabel('Predicted')
    axes[0].set_ylabel('True')
    
    cm_s = confusion_matrix(results['shape_labels'], results['shape_preds'])
    sns.heatmap(cm_s, annot=True, fmt='d', cmap='Reds', ax=axes[1],
                xticklabels=encoders[1].classes_, yticklabels=encoders[1].classes_)
    axes[1].set_title('Shape (Single-Location)', fontsize=14, fontweight='bold')
    axes[1].set_xlabel('Predicted')
    axes[1].set_ylabel('True')
    
    cm_g = confusion_matrix(results['grading_labels'], results['grading_preds'])
    sns.heatmap(cm_g, annot=True, fmt='d', cmap='Reds', ax=axes[2],
                xticklabels=encoders[2].classes_, yticklabels=encoders[2].classes_)
    axes[2].set_title('Grading (Single-Location)', fontsize=14, fontweight='bold')
...
def plot_history(history, save_path):
    ...
    axes[0,0].set_title('Loss (Single-Location)', fontweight='bold')
    axes[0,0].legend()
...
def main():
    print("="*80)
    print(f"Single-Location Baseline Training ({Config.TARGET_LOCATION} only)")
    print("="*80)
    ...
    full_dataset = SingleLocationDataset(df, Config.DATA_DIR, Config.TARGET_LOCATION, augment=False)
    print(f"Valid samples (with {Config.TARGET_LOCATION}): {len(full_dataset)}\n")
    ...
    try:
        ...
        print("⚠️ Using random split")
    
    print(f"Split: Train={len(train_idx)}, Val={len(val_idx)}, Test={len(test_idx)}\n")
    
    train_ds = SingleLocationDataset(df.iloc[train_idx], Config.DATA_DIR, Config.TARGET_LOCATION, augment=True)
    val_ds = SingleLocationDataset(df.iloc[val_idx], Config.DATA_DIR, Config.TARGET_LOCATION, augment=False)
    test_ds = SingleLocationDataset(df.iloc[test_idx], Config.DATA_DIR, Config.TARGET_LOCATION, augment=False)
    ...
    model = SingleLocationCNN(len(enc_t.classes_), len(enc_s.classes_), len(enc_g.classes_)).to(Config.DEVICE)
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        print(f"Timing  F1: {train_t_f1:.4f}/{val_res['timing_f1']:.4f}")
        print(f"Shape   F1: {train_s_f1:.4f}/{val_res['shape_f1']:.4f}")
        print(f"Grading F1: {train_g_f1:.4f}/{val_res['grading_f1']:.4f}")
        ...
        if avg_f1 > best_f1:
            ...
            torch.save({'model': model.state_dict(), 'f1': avg_f1}, 
                      os.path.join(Config.OUTPUT_DIR, 'best_model_single.pth'))
...
if __name__ == "__main__":
    ...
    results_df.to_csv(os.path.join(Config.OUTPUT_DIR, 'test_results_single.csv'), index=False)
    
    plot_confusion_matrices(test_res, encoders, os.path.join(Config.OUTPUT_DIR, 'confusion_matrices_single.png'))
    plot_history(history, os.path.join(Config.OUTPUT_DIR, 'training_history_single.png'))
